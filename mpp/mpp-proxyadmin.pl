...
BEGIN {
    ...
    $failover_cachefile = "/usr/local/mpp/cache/mpp-cache";  # The default cache file
}
...
HELP_EOF
    #- Options to implement, or some version of them.
...
GetOptions( \%options,
        ...
        "proxyhost=s",  "proxyport=s",  "proxyuser=s", "proxypass=s",
...
$proxyhost  = new mysqlpool::host::mysqlproxy( host => $options{'proxyhost'}, port => $options{'proxyport'},
                                                username => $options{'proxyuser'}, password => $options{'proxypass'} );
...
if ((defined $options{'initialize-all'}) || (defined $options{'set-all'})) {
    ...
    my $a = $proxyhost->show("ALL") || die $proxyhost->errormsg(),"\n";
    my $indent = {};
